...
func (node *Node) Start(exitChannels ...*chan os.Signal) {
	...
	node.Server, err, shouldRestart = lib.NewServer(
		node.Config.Params, listeners, desoAddrMgr, connectIPs, node.chainDB, nil,
		targetOutboundPeers, maxInboundPeers, node.Config.MinerPublicKeys, minerCount,
		true, node.Config.HyperSync, node.Config.SyncType, node.Config.MaxSyncBlockHeight,
		false, rateLimitFeerateNanosPerKB, MinFeeRateNanosPerKB, stallTimeoutSeconds, maxBlockTemplatesToCache,
		minBlockUpdateInterval, blockCypherAPIKey, true, lib.SnapshotBlockHeightPeriod,
		node.Config.DataDirectory, mempoolDumpDir, disableNetworking, readOnly,
		false, nil, "", []string{}, 0,
		node.EventManager, node.nodeMessageChan, node.Config.ForceCheksum)
	if err != nil {
		if shouldRestart {
