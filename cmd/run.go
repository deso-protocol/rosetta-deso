...
to quickly create a Cobra application.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		node := bitclout.NewNode(config.Mode == configuration.Online, config.DataDirectory, config.Params, config.NodePort, config.MinerPublicKeys)
		go node.Start()
...
func init() {
	runCmd.PersistentFlags().String("network", string(configuration.Mainnet), "network to connect to")
	runCmd.PersistentFlags().String("mode", string(configuration.Online), "mode to start in")
